...
namespace Ilaro.Admin.Core.Configuration.Configurators
{
    internal class ConfiguratorsHolder : IConfiguratorsHolder
    {
        ...
        private IDictionary<MemberInfo, PropertyConfigurationHolder> _propertyCustomizers { get; } =
            new Dictionary<MemberInfo, PropertyConfigurationHolder>();

        public ConfiguratorsHolder(Type type)
        {
            Type = type;
            foreach (var member in Type.GetProperties())
            {
                _propertyCustomizers[member] = new PropertyConfigurationHolder();
            }
        }
        ...
        public void Property(MemberInfo memberOf, Action<IPropertyConfigurator> customizer)
        {
            customizer(new PropertyConfigurator(GetPropertyCustomizer(memberOf)));
        }

        internal PropertyConfigurationHolder GetPropertyCustomizer(MemberInfo memberInfo)
        {
            var propertyInfo = (PropertyInfo)memberInfo;
            if (_propertyCustomizers.ContainsKey(propertyInfo) == false)
            {
                _propertyCustomizers[propertyInfo] = new PropertyConfigurationHolder();
            }
