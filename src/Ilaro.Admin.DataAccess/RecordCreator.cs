    ...
    public class RecordCreator : IRecordCreator
    {
        ...
        private DbCommand CreateBaseCommand(EntityRecord entityRecord)
        {
            ...
            foreach (var propertyValue in entityRecord.Values
                .WhereIsNotSkipped()
                .WhereIsNotOneToMany()
                .DistinctBy(x => x.Metadata.Column)
                .Where(x => x.Metadata.IsAutoKey == false))
            {
                sbColumns.AppendFormat("{0},", propertyValue.Metadata.Column);
            ...
            if (entityRecord.Keys.Count > 1 || entityRecord.Keys.FirstOrDefault().Metadata.TypeInfo.IsString)
            {
                idType = "nvarchar(max)";
                insertedId = "@" + counter;
                cmd.AddParam(entityRecord.JoinedKeysValues);
            }
            var table = entityRecord.Metadata.Table;
...
-- update foreign entities records";
        ...
        private void AddForeignsUpdate(DbCommand cmd, EntityRecord entityRecord)
        {
            ...
            foreach (var propertyValue in entityRecord.Values
                .WhereOneToMany()
                .Where(x => x.Metadata.IsManyToMany == false)
                .Where(value => value.Values.IsNullOrEmpty() == false))
            {
                ...
                if (addSqlPart)
                {
                    ...
                    var foreignKey = entityRecord.Metadata.Keys.FirstOrDefault().Column;
 ...
 WHERE {constraints};");
        ...
        private void AddManyToManyForeignsUpdate(DbCommand cmd, EntityRecord entityRecord)
        {
            ...
            foreach (var propertyValue in entityRecord.Values.WhereOneToMany()
                .Where(x => x.Metadata.IsManyToMany))
            {
